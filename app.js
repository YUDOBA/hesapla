...
function halfHtml(p) {
  ...
  html += '<div class="actions">' +
    ...
    '<button class="btn pas"' + (pasOn?'':' disabled') + '>PAS</button></div></div>';
  return html;
}
function midHtml() {
  var ops = ['\u2212','+','\u00d7','\u00f7'];
  ...
  return '<div class="mid">' + col(ops.slice(0,2)) +
    '<div class="well">' + dieHtml(0) + dieHtml(1) +
...
function finishGame() {
  ...
  if (state.wins[0] !== state.wins[1]) {
    ...
    title = state.nick[w2] + ' kazandi';
    lines = ['Tur esit', 'Dusuk ceza: ' + state.penalty[w2]];
  } else {
    title = 'Berabere';
    lines = ['Tur ' + state.wins[0] + '-' + state.wins[1], 'Ceza ' + state.penalty[0] + '-' + state.penalty[1]];
  }
